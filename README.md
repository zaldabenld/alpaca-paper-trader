...
For strategy research, only the stock-selection layer should change. In the dashboard, this means the trade-profile controls for entry thresholds, feature weights, inverse ETF behavior, and exits are the strategy surface. The replay sizing/capacity/account controls are the base harness and should stay fixed for comparisons: default 20 max positions and 20 sizing slots. Do not tune account size, buying power, trade dollars, exposure, max positions, or share price to decide what to buy. If those need testing, label the run as a separate sizing/capacity diagnostic after a strategy is already selected.
...
The dashboard is localhost-only and runs replay jobs in the background through `strategy_simulation_hub.py`. It does not connect to Alpaca, change credentials, or alter saved account settings. The default mode is **New trade profile**: adjust entry thresholds, selection-weight toggles/sliders, exit settings, and inverse ETF mode directly in the dashboard, then run the simulation. Existing strategies can still be selected as seeds, but they are no longer required. With **Use trade profile controls** enabled, each run writes a temporary job config under `reports\manual\dashboard-jobs\` and leaves saved research configs unchanged. Use the **Bar Screen** preset for fast candidate shortlisting, then the **Trade Validate** preset for trade-price replay at 5/10/15 bps. Keep the lower replay harness controls at the base defaults for strategy comparisons unless the run is explicitly a sizing/capacity diagnostic. Dashboard job logs are stored in `reports\manual\dashboard-jobs\`.
